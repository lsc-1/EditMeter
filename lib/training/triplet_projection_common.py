...
from lib_sep_4methond_distance_logall_0.exp_triplet import label_id_to_name
        ...
        for batch in tqdm.tqdm(dataloader, desc=desc):
            ...
            for index in range(len(labels)):
                ...
                for key in (
                    ...
                    row[key] = get_batch_float_metadata(
                        batch,
                        key,
                        index,
                    )
...
def evaluate_two_models_and_save_test(
    ...
    payload = {
        ...
        "human_f1": cls_metrics.get("human_f1"),
        "edited_f1": cls_metrics.get("edited_f1"),
        "ai_f1": cls_metrics.get("ai_f1"),

        "confusion_matrix": (
            cls_metrics["confusion_matrix"].tolist()
        ),

        "classification_report": (
            cls_metrics["classification_report_dict"]
        ),
        ...
        "pred_degree_stats_by_true_label": (
            compute_pred_degree_stats_by_label(details)
        ),
    ...
    if output_dir:
        path = save_results(
            output_dir,
            "triplet_projection_results.json",
            payload,
        )
